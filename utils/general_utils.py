...
def _collate_survpath(batch):
    ...
    """
    # per batch: [img-256dim, pathways-256dim, label-dss, event_time, censorship, clinical, mask]
    img = torch.stack([item[0] for item in batch]) 

    omic_data_list = []
    for item in batch:
        omic_data_list.append(item[1])
    # print("omic_data_list: ", omic_data_list)# len=1
...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    elif args.modality in ["abmil_wsi", "abmil_wsi_pathways", "clam_wsi", "clam_wsi_pathways", "deepmisl_wsi", "deepmisl_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways"]:
