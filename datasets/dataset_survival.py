...
class SurvivalDatasetFactory:
    ...
    def __init__(self,
        ...
        lora_r = 4,  
        lora_ahpha = 16, # here
        our_fusion = 'concat' # here, concat or bilinear
        ):
        r"""
        ...
        self.fourier_hidden = fourier_hidden
        self.gridsize = gridsize
        self.wsi_projection_dim = wsi_projection_dim
        self.mae_weight = mae_weight
        self.lora_r = lora_r  
        self.lora_ahpha = lora_ahpha 
        self.our_fusion = our_fusion 
    ...
    def _setup_survpath(self):
       ...
       # running with hallmarks, reactome, and combined signatures
        number = self.per_pathway_gene_number_over_n # default=0, you can set n = 10/ 50/ 100
        if number:
            self.signatures = pd.read_csv("./datasets_csv/metadata/{}_signatures_over_{}.csv".format(self.type_of_path, number))
        else:
            self.signatures = pd.read_csv("./datasets_csv/metadata/{}_signatures.csv".format(self.type_of_path))

        self.omic_names = []
        for col in self.signatures.columns: #(genes, pathways)
            omic = self.signatures[col].dropna().unique() 
            omic = sorted(_series_intersection(omic, self.all_modalities["rna"].columns)) 
            self.omic_names.append(omic)
        self.omic_sizes = [len(omic) for omic in self.omic_names]
    ...
    def _setup_omics_data(self):
        ...
        """
        self.all_modalities = {}
        for modality in ALL_MODALITIES: # rna_clean.csv
    ...
    def _get_label_dict(self):
        ...
        print("label_dict keys : ", label_dict.keys)
    ...
    def _summarize(self):
        ...
        if self.print_info:
            ...
            print("number of cases --label_data {}".format(len(self.label_data)))
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str='train', fold = None, scaler=None, valid_cols=None):
        ...
        # select the rna data for this split
        ...
        for key in args.dataset_factory.all_modalities.keys(): 
            ...
            if split_key in ["val"]:
                ...
                num_feats = df_for_norm.shape[1] # 4999
                columns = {}
            ...
            omics_data_for_split[key] = filtered_normed_df # rna_df
        ...
        split_dataset = SurvivalDataset(
            ...
            omics_data_dict=omics_data_for_split,# rna_df(patient, 4999-gene)
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.modality in ["mlp_per_path_wsi", "abmil_wsi", "abmil_wsi_pathways", "clam_wsi", "clam_wsi_pathways","deepmisl_wsi", "deepmisl_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways"]:
...
# ---------------------------------------------------- -------------------------------------------------------------------------------------- here ---------------- # 
        elif self.modality in ["snn_chebykan_snn_dual_attn_lora", 
            ...
            return (patch_features, omic_list, label, event_time, c, clinical_data, mask) # data[5]=clinical_data
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        patch_features = torch.cat(patch_features, dim=0) # [n_patches_per_slide, 1024]

        if self.sample: # train set
            max_patches = self.num_patches # 4096, in order to speed up the process of training

            n_samples = min(patch_features.shape[0], max_patches) # patch_features.shape[0]
