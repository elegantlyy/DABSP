...
class AgentAttention(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        if sr_ratio > 1:
            self.sr = nn.Conv2d(dim, dim, kernel_size=sr_ratio, stride=sr_ratio) # 在特征图上应用空间缩减
            self.norm = nn.LayerNorm(dim)

        self.agent_num = agent_num
        self.dwc = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=(3, 3), padding=1, groups=dim) # 深度可分离卷积层，可以有效地捕捉局部特征，用于在注意力输出和特征图之间进行交叉
        ...
        trunc_normal_(self.an_bias, std=.02) # 使用截断正态分布初始化偏置参数
    ...
    def forward(self, x, H, W):
        b, n, c = x.shape # (1, 331, 256) / (1, 4096, 256)
        ...
        agent_tokens = self.pool(q.reshape(b, H, W, c).permute(0, 3, 1, 2)).reshape(b, c, -1).permute(0, 2, 1) # （b,n,c）
        ...
        position_bias1 = nn.functional.interpolate(self.an_bias, size=kv_size, mode='bilinear') # # 通常用于改变图像特征图的大小
...
class AgentAttentionForSequences(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 agent_num=49, **kwargs):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        self.agent_num = agent_num
        # self.dwc = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=(3, 3), padding=1, groups=dim) # 深度可分离卷积层，用于在注意力输出和特征图之间进行交叉
        self.an_bias = nn.Parameter(torch.zeros(num_heads, agent_num, 7, 7))
        self.na_bias = nn.Parameter(torch.zeros(num_heads, agent_num, 7, 7))
    ...
    def forward(self, x):
        ...
        agent_tokens = self.pool(q.reshape(b, n, c))
        ...
        # self.an_bias： torch.zeros(num_heads, agent_num, 7, 7)
        position_bias = self.an_bias.unsqueeze(0).unsqueeze(0).repeat(b, 1, 1, 1)

        agent_attn = self.softmax((agent_tokens * self.scale) @ k.transpose(-2, -1) + position_bias)
        agent_attn = self.attn_drop(agent_attn)
        agent_v = agent_attn @ v

        agent_bias = self.na_bias.unsqueeze(0).unsqueeze(0).repeat(b, 1, 1, 1)
...
class MultimodalityAgentAttentionForSequences(nn.Module):
    def __init__(self, dim, num_patches, num_pathways = 281, num_heads=8, dim_head = 64, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 agent_num=49, residual = True, residual_conv_kernel = 33,  eps = 1e-8, dropout = 0., return_attention=False, **kwargs):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        self.q = nn.Linear(dim, dim, bias=qkv_bias)
        self.kv = nn.Linear(dim, dim * 2, bias=qkv_bias) # q 单独的， kv 参数共享
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

        self.agent_num = agent_num # 49
        # self.dwc = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=(3, 3), padding=1, groups=dim) # 深度可分离卷积层，用于在注意力输出和特征图之间进行交叉
    ...
    def forward(self, x, mask=None, return_attention=False):
        # img 生成 q,k,v    img生成的q生成pathway的q
        b, n, c, h, m, eps = *x.shape, self.num_heads, self.num_pathways, self.eps
        # b, n, c = x.shape # (1, 331+4096, 256)
        # print("b, n, c, h, m", b, n, c, h, m) # 1, 4427, 256, 8, 281
        num_heads = self.num_heads # 8
        head_dim = c // num_heads # 256/8=32
        # print(f"mun_heads: {num_heads}, head_dim: {head_dim}")
        ...
        q = q * self.scale 
        q = q.reshape(b, n, num_heads, head_dim).permute(0, 2, 1, 3) # (b, num_heads, n, head_dim)
        print("q.shape-img before extract modality qkv: ", q.shape) # torch.Size([1, 8, 4427, 32])
        ...
        v_histology = v[:, :, self.num_pathways:, :]  # bs x head x num_patches x dim

        q_pathways = q[:, :, :self.num_pathways, :]  # bs x head x num_pathways x dim
        k_pathways = k[:, :, :self.num_pathways, :]
        v_pathways = v[:, :, :self.num_pathways, :]  # bs x head x num_pathways x dim

        print("q_pathways shape: ", q_pathways.shape)
      
        # method 1， generate x_h
        pathway_attn = self.softmax((q_pathways * self.scale) @ k_histology.transpose(-2, -1) ) # (bs, head, 331, 4096)
        pathway_attn = self.attn_drop(pathway_attn)
        pathway_v = pathway_attn @ v_histology

        q_attn = self.softmax((q_histology * self.scale) @ q_pathways.transpose(-2, -1) )
        q_attn = self.attn_drop(q_attn)
        x_h = q_attn @ pathway_v # (bs, head, 4096, 256)
        print("x_h shape: ", x_h.shape)

        x_h = x_h.transpose(1, 2).reshape(b, n, c)

        # method 2， generate x_p
        # x_h = x_h.transpose(1, 2).reshape(b, n, c)
        x = self.proj(x_h)
        x = self.proj_drop(x)
        print("x.shape: ", x.shape)
        if return_attention:
            x, attn_pathways, cross_attn_pathways, cross_attn_histology = self.attn(x=self.norm(x), mask=mask, return_attention=True)
